<?php
/* Taken from http://www.phpclasses.org/package/6279-PHP-Parse-and-retrieve-data-from-Excel-XLS-files.html
 * 
...
// 			return false;
		...
		foreach ($ws->sheetData->row[$row] as $c) {
			list($curC,) = $this->_columnIndex((string) $c['r']);
			$ret_rows[ $curC ] = $this->value($c);
		}
		for ($i = 0; $i < $cols; $i++)
		if (!isset($ret_rows[$i]))
			$ret_rows[ $i ] = '';
...
//		$this->styles = array();
		...
		if ( $relations = $this->getEntryXML("_rels/.rels" ) ) {
			...
			foreach ($relations->Relationship as $rel) {
				...
				if ($rel["Type"] == ExcelMgr_SimpleXLSX::SCHEMA_REL_OFFICEDOCUMENT) {
...
//							echo 'workbook relations<br />';
							...
							foreach ($workbookRelations->Relationship as $workbookRelation) {
								...
								if ($workbookRelation['Type'] == ExcelMgr_SimpleXLSX::SCHEMA_REL_WORKSHEET) { // Sheets
...
//										echo '<pre>'.htmlspecialchars( print_r( $sheet, true ) ).'</pre>';
								...
								} else if ($workbookRelation['Type'] == ExcelMgr_SimpleXLSX::SCHEMA_REL_SHAREDSTRINGS && $this->entryExists( $path )) { // 0.6.6
...
//									echo 'sharedstrings<br />';
								...
								} else if ($workbookRelation['Type'] == ExcelMgr_SimpleXLSX::SCHEMA_REL_STYLES) {
